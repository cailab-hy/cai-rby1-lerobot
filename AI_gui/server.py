...
STATIC_FILES = {
    ...
    "vad-worklet.js": "text/javascript; charset=utf-8",
}
...
def load_config() -> dict[str, Any]:
    ...
    languages = spoken_languages(config)
    if not languages:
        raise ValueError("stt.languages must list at least one language")
    for task in tasks:
        for field in ("voice_keywords", "voice_response"):
            value = task.get(field)
            if not isinstance(value, dict):
                raise ValueError(
                    f"Task '{task['id']}' needs {field} as an object keyed by language, e.g. "
                    f'{{"ko": ..., "en": ...}}'
                )
            missing_languages = [language for language in languages if not value.get(language)]
            if missing_languages:
                raise ValueError(
                    f"Task '{task['id']}' {field} is missing: {', '.join(missing_languages)}"
                )
    for group in ("action_words", "pause_words", "terminate_words"):
        value = config.get("voice", {}).get(group)
        if not isinstance(value, dict) or any(not value.get(language) for language in languages):
            raise ValueError(f"voice.{group} needs a non-empty list for each of: {', '.join(languages)}")
...
def spoken_languages(config: dict[str, Any]) -> list[str]:
    languages = config.get("stt", {}).get("languages", list(DEFAULT_LANGUAGES))
    return [str(language) for language in languages if language]
...
def client_phase_from_message(message: str) -> str | None:
    if "Sending policy instructions to policy server" in message:
        return "policy_loading"
    if "Starting gRPC action receiver thread" in message or (
        "Starting " in message and " ZMQ action receiver thread" in message
    ):
        return "executing"
    if "Moving to ready pose" in message:
...
def validate(config: dict[str, Any]) -> list[str]:
    ...
    # faster_whisper is deliberately not required here: speech recognition runs
    # as a separate service on the operator's machine (see stt_service.py).
...
class CameraStream:
    def __init__(
        ...
        self.version = 0
        self.state = "idle"
        # The browser renders camera status in the operator's current language,
        # so the server only reports a key plus its parameters.
        self.message_key = "camera.idle"
    ...
    def _set_state(self, state: str, message_key: str, **params: Any) -> None:
        with self.condition:
            self.state = state
    ...
    def _run(self) -> None:
        ...
        started = False
        try:
            self._set_state("connecting", "camera.connecting")
            pipeline.start(rs_config)
            started = True
            self._set_state("connected", "camera.connected", fps=self.fps)
            ...
            self._set_state("error", "camera.failed", error=str(exc))
...
class RobotClientManager:
    def __init__(
        ...
        self.active_instruction: str | None = None
        self.control_path = self.relay.directory / "ai_control.json"
        self.control_revision = 0
        self.shutdown_event = threading.Event()

    def _write_control_locked(self, *, paused: bool, task_id: str, instruction: str) -> None:
    ...
    def start(self) -> tuple[bool, str]:
        ...
        try:
            ...
            environment["RBY1_AI_CONTROL_FILE"] = str(self.control_path)
            with self.lock:
                self._write_control_locked(
                    paused=False,
        ...
        with self.lock:
            ...
            self.state = "running"
            self.phase = "robot_preparing"
            self.active_task_id = selected_task["id"]
            self.active_instruction = selected_task["instruction"]
    ...
    def _read_process(self, process: subprocess.Popen[str]) -> None:
        ...
        try:
            for line in process.stdout:
                message = line.rstrip("\n")
                self.event_log.add(message, "client")
                phase = client_phase_from_message(message)
                if phase is not None:
                    with self.lock:
                        if self.process is process and self.state == "running":
                            self.phase = phase
        finally:
    ...
    def pause(self) -> tuple[bool, str]:
        with self.lock:
            process = self.process
            if not process or process.poll() is not None or self.state != "running":
                return False, "Robot client is not running."
            if not self.active_task_id or not self.active_instruction:
                return False, "Active task information is unavailable."
            self._write_control_locked(
                paused=True,
            ...
            self.state = "paused"
            self.phase = "paused"
        self.event_log.add("Policy execution paused; robot client and cameras remain connected.")
        return True, "Policy execution paused."

    def resume(self) -> tuple[bool, str]:
        selected_task = self.task_provider()
        with self.lock:
            process = self.process
            if not process or process.poll() is not None or self.state != "paused":
                return False, "Robot client is not paused."
            self._write_control_locked(
                paused=False,
                task_id=selected_task["id"],
                instruction=selected_task["instruction"],
        ...
        self.event_log.add(
            f"Policy execution resumed with task {selected_task['number']}. {selected_task['label']}."
        )
        return True, "Policy execution resumed."
...
class WebApplication:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.languages = spoken_languages(config)
        self.stop_event = threading.Event()
    ...
    def select_task(self, task_id: str) -> tuple[bool, str]:
        if self.client.status()["state"] in {"starting", "running", "stopping"}:
    ...
    def client_active(self) -> bool:
        return self.client.status()["state"] in {"starting", "running", "paused", "stopping"}
    ...
    def camera_statuses(self) -> list[dict[str, Any]]:
        ...
        if self.client_active():
            ...
            for key in self.policy_camera_keys:
                ...
                status["message_key"] = "camera.relay_live" if fresh else "camera.relay_wait"
                status["message_params"] = {"fps": status["fps"]} if fresh else {}
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "version": self.config.get("web_version", "ai-v3"),
            "languages": list(self.languages),
            "voice": self.config.get("voice", {}),
            # Speech recognition runs beside the microphone, not here; the
            # browser talks to it directly and reports its health.
            "stt_endpoint": self.config.get("stt", {}).get(
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if parsed.path == "/":
            self._send_file(STATIC_DIR / "index.html", STATIC_FILES["index.html"])
        elif parsed.path.lstrip("/") in STATIC_FILES:
            name = parsed.path.lstrip("/")
            self._send_file(STATIC_DIR / name, STATIC_FILES[name])
    ...
    def do_POST(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        if self.headers.get("X-RBY1-Control") != "ai-v3":
            self._send_json(
        ...
        elif parsed.path == "/api/client/pause":
            ok, message = self.server.app.client.pause()
            self._send_json({"ok": ok, "message": message}, HTTPStatus.OK if ok else HTTPStatus.CONFLICT)
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    if args.check:
        print("AI Voice GUI V3 check passed")
    ...
    app.log.add("RB-Y1 AI Voice GUI V3 server started.")
    app.cameras.start_all()

    print("RB-Y1 AI Voice GUI V3 is running.")
    ...
    try:
        ...
        print("\nStopping RB-Y1 AI Voice GUI V3...")
    finally:
        app.stop_event.set()
        server.shutdown()
        server.server_close()
